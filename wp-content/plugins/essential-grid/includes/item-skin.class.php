		...
		if($link_set_to !== 'none'){
			switch($link_type_link){
				case 'post':
					if($demo === false){
						if($is_post){
							$link_wrapper = '<a href="'.get_permalink( $this->post['ID'] ).'">%REPLACE%</a>';
						}else{
							$get_link = $this->get_custom_element_value('post-link', '', ''); //get the post link
							if($get_link == '')
								$link_wrapper = '<a href="javascript:void(0);">%REPLACE%</a>';
							else
								$link_wrapper = '<a href="'.$get_link.'">%REPLACE%</a>';
						}
							
					}else{
						$link_wrapper = '<a href="javasccript:void(0);">%REPLACE%</a>';
					}
				break;
				case 'url':
					$link_wrapper = '<a href="'.$base->getVar($this->params, 'link-url-link', 'javascript:void(0);').'">%REPLACE%</a>';
				break;
				case 'meta':
					if($demo === false){
						if($is_post){
							...
							if($meta_link == ''){// if empty, link to nothing
								...
								$link_wrapper = '<a href="'.$meta_link.'">%REPLACE%</a>';
							}
						}else{
							$get_link = $this->get_custom_element_value('post-link', '', ''); //get the post link
							if($get_link == '')
				...
				break;
				case 'javascript':
					$link_wrapper = '<a href="javascript:'.$base->getVar($this->params, 'link-javascript-link', 'void(0);').'">%REPLACE%</a>';
				break;
				case 'lightbox':
	...
	public function insert_layer($layer, $demo = false, $masonry = false){
		...
		switch($link_to){
			case 'post':
				if($demo === false){
					if($is_post){
						$text = '<a href="'.get_permalink( $post['ID'] ).'">'.$text.'</a>';
					}else{
						$get_link = $this->get_custom_element_value('post-link', $separator, ''); //get the post link
						if($get_link == '')
							$text = '<a href="javascript:void(0);">'.$text.'</a>';
						else
							$text = '<a href="'.$get_link.'">'.$text.'</a>';
					}
						
				}else{
					$text = '<a href="javasccript:void(0);">'.$text.'</a>';
				}
			break;
			case 'url':
				$text = '<a href="'.$base->getVar($layer['settings'], 'link-type-url', 'javascript:void(0);').'">'.$text.'</a>';
			break;
			case 'meta':
				if($demo === false){
					if($is_post){
			...
			break;
			case 'javascript':
				$text = '<a href="javascript:'.$base->getVar($layer['settings'], 'javascript-link', 'void(0);').'">'.$text.'</a>';
			break;
			case 'lightbox':
				wp_enqueue_script('themepunchboxext');
				wp_enqueue_style('themepunchboxextcss');
	...
	public function get_post_value($handle, $separator, $meta){
		...
		switch($handle){
			...
			case 'excerpt':
				...
				$text =($text); //,"<b><br><br/><i><strong><small>"
				break;
			case 'meta':
				$m = new Essential_Grid_Meta();
			...
			case 'cat_list':
				...
				break;
			case 'tag_list':
				$text = get_tags_html_list($this->post['ID']);	
				break;
