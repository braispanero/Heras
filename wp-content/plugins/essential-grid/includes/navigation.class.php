    ...
    public function output_filter($demo = false){
        ...
        if(!empty($this->filter)){
            foreach($this->filter as $filter_id => $filter){
                ...
                echo '<div class="esg-filterbutton" data-fid="'.$filter_id.'" data-filter="filter-'.sanitize_title($filter['slug']).'"><span>'.$filter_text.'</span><span class="esg-filter-checked"><i class="eg-icon-ok-1"></i></span></div>';
            }
        }
        echo '</div>';
