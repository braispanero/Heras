					...
					if(!empty($custom_meta)){
						...
						foreach($custom_meta as $cmeta){
							...
							<div class="eg-elset-row"><div class="eg-elset-label"  class="eg-mb-label"><?php echo $cmeta['name']; ?>:</div>
								...
								switch($cmeta['type']){
									case 'text':
										echo '<input type="text" name="eg-cm-'.$cmeta['handle'].'" value="" />';
										break;
									case 'select':
										$el = $meta->prepare_select_by_string($cmeta['select']);
										echo '<select name="eg-cm-'.$cmeta['handle'].'">';
									...
									case 'image':
										$var_src = '';
										?>
										<input type="hidden" value="" name="eg-cm-<?php echo $cmeta['handle']; ?>" id="eg-cm-<?php echo $cmeta['handle']; ?>" />
										<a class="button-primary revblue eg-cm-image-add" href="javascript:void(0);" data-setto="eg-cm-<?php echo $cmeta['handle']; ?>"><?php _e('Choose Image', EG_TEXTDOMAIN); ?></a>
										<a class="button-primary revred eg-cm-image-clear" href="javascript:void(0);" data-setto="eg-cm-<?php echo $cmeta['handle']; ?>"><?php _e('Remove Image', EG_TEXTDOMAIN); ?></a>
										<div>
											<img id="eg-cm-<?php echo $cmeta['handle']; ?>-img" src="<?php echo $var_src; ?>" <?php echo ($var_src == '') ? 'style="max-width:200px; display: none;margin:20px 0px 0px 250px;"' : ''; ?>>
										</div>
										<?php
										break;
								}
