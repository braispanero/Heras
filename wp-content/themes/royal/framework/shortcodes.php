...
function etheme_gallery_shortcode( $attr ) {
	...
	foreach ( $attachments as $id => $attachment ) {
		...
		if ( ! empty( $link ) && 'file' === $link )
			$image_output = wp_get_attachment_link( $id, $size, false, array('class'=>'pphoto') );
		elseif ( ! empty( $link ) && 'none' === $link )
			$image_output = wp_get_attachment_image( $id, $size, false, array('rel'=>'pphoto') );
		else
			$image_output = wp_get_attachment_link( $id, $size, true, array('rel'=>'pphoto') );
		...
		$output .= "
			<{$icontag} data-index='".$i."' class='gallery-icon {$orientation}'>
				$image_output
			</{$icontag}>";
...
function etheme_banner_shortcode($atts, $content) {
    ...
    return '<div class="banner '.$a['class'].' banner-font-'.$a['font_style'].' hover-'.$a['hover'].'" '.$onclick.'><div class="banner-content"><div class="banner-inner">'.do_shortcode($content).'</div></div><img src="'.$src.'"/></div>';
}
